...
// Функция для получения данных о подписчиках YouTube
async function fetchYouTubeSubscribers() {
    ...
    try {
        ...
        // Парсим HTML для поиска количества подписчиков
        const subscriberMatch = html.match(/"subscriberCountText":\{"simpleText":"([\d,]+) подписчиков"\}/);
        
        if (subscriberMatch && subscriberMatch[1]) {
            const subscribers = subscriberMatch[1].replace(/,/g, '');
            document.getElementById('subscriberCount').textContent = formatNumber(subscribers);
        } else {
            // Альтернативный метод поиска
            const altMatch = html.match(/"subscriberCountText":\{"runs":\[\{"text":"([\d,]+)"\},\{"text":" подписчиков"\}\]\}/);
            if (altMatch && altMatch[1]) {
                const subscribers = altMatch[1].replace(/,/g, '');
                document.getElementById('subscriberCount').textContent = formatNumber(subscribers);
            } else {
                throw new Error('Не удалось найти данные о подписчиках');
            }
        }
    } catch (error) {
        console.error('Ошибка при получении данных YouTube:', error);
        // Запасной вариант - статическое число (замените на ваше реальное количество)
...
// Функция для форматирования чисел (без округления)
function formatNumber(num) {
    return parseInt(num).toLocaleString('ru-RU');
}

// В функции initializeApp() добавьте вызов:
function initializeApp() {
    // ... существующий код ...
    
    // Загружаем данные о подписчиках
    fetchYouTubeSubscribers(); // Используйте эту функцию для парсинга HTML
    // ИЛИ используйте API версию (нужен API ключ):
    // fetchYouTubeSubscribersAPI();
    
    // ... остальной код ...
}

// Обновите переводы для нового текста
function applyLanguage(lang) {
    const translations = {
        ru: {
    ...
    // ... существующий код переводов ...
    
    // Обновляем текст счетчика подписчиков
    document.querySelectorAll('.counter-label').forEach(element => {
        if (element.getAttribute('data-i18n') === 'subscribers') {
            element.textContent = t.subscribers;
        }
    });
}
